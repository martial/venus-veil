...
export async function recordProjectedFrame(projector, { attempts = 4, pause = ms => new Promise(resolve => setTimeout(resolve, ms)) } = {}) {
  for (let attempt = 0; attempt < attempts; attempt++) {
    if (projector.state.status !== 'ready') await projector.health();
    if (projector.state.status === 'ready') {
      const frameId = await projector.frame({ strict: true });
